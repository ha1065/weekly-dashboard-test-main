#!/usr/bin/env python3
"""Apply vw_kpi_ytd rebuild by deploying updated Lambda and running migration."""
...
def invoke(payload):
    r = lc.invoke(FunctionName='production-clockify-import', Payload=json.dumps(payload).encode())
    return json.loads(r['Payload'].read())

# Build the CREATE VIEW SQL as a single string (no semicolons inside)
view_sql = """CREATE OR REPLACE VIEW vw_kpi_ytd AS
WITH ordered AS (
    SELECT *
...
SELECT
    ...
    target_ps_on_time_pct, target_ps_avg_duration_weeks,
    ...
    avg_escalation_days_open, escalations_resolved_ytd, active_resource_count
FROM ordered"""

# Use run_escalations_import pattern — send raw DDL via a custom mode
# Actually use the existing run_query which handles CREATE statements
print('Creating view...')
result = invoke({'mode': 'run_query', 'sql': view_sql})
print(f'Status: {result.get("statusCode")}')
if result.get('errorMessage'):
    print(f'ERROR: {result["errorMessage"][:400]}')
    exit(1)

# Verify
print('Verifying...')
check = invoke({'mode': 'run_query', 'sql': 'SELECT week_start_date, billable_util_prev, ps_active_prev FROM vw_kpi_ytd ORDER BY week_start_date DESC LIMIT 1'})
print(check)

# Refresh SPICE
qs.create_ingestion(AwsAccountId=ACCOUNT, DataSetId='kpi-weekly-snapshots-prod',
    IngestionId=f'mig060c-{int(time.time())}')
